...
class Network(torch.nn.Module):
	def __init__(self):
		...
		self.load_state_dict(torch.load(abs_path + '/network-' + arguments_strModel + '.pytorch'))
	# end
...
def edge_detector(file_path):
	tensorInput = torch.FloatTensor(numpy.array(PIL.Image.open(file_path))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
	tensorOutput = estimate(tensorInput)
	img = (tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)
	return img
# end