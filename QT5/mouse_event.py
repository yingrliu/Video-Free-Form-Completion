...
Author: Yingru Liu
paint area.
copy from https://github.com/run-youngjoo/SC-FEGAN/tree/master/ui
"""
...
class GraphicsScene(QGraphicsScene):
    def __init__(self, mode_list, parent=None):
        ...
        self.modes = mode_list              # mode_list = [is_mask, is_sketch, is_color]
    ...
    def mouseMoveEvent(self, event):
        if self.mouse_clicked:
            if self.modes[0] == 1:
                ...
                if self.prev_pt:
                    self.drawStroke(self.prev_pt, event.scenePos())                         # color
                    pts = {}
                    pts['prev'] = (int(self.prev_pt.x()), int(self.prev_pt.y()))
                    pts['curr'] = (int(event.scenePos().x()), int(event.scenePos().y()))
    ...
    def drawStroke(self, prev_pt, curr_pt):
        lineItem = QGraphicsLineItem(QLineF(prev_pt, curr_pt))
        lineItem.setPen(QPen(QColor(self.stk_color), 10, Qt.SolidLine))  # rect
        self.addItem(lineItem)
